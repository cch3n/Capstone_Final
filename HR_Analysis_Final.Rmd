...
```{r fig.width=10, message=TRUE, warning=FALSE, echo=FALSE}
...
* Bi-modal relationship between quitting and company evaluation. * The company is losing many of their top evaluated performers. 
...
proplot <- ggplot(testingpromotion, aes(promotion, pct, colour = Quit, fill = Quit)) +
  geom_bar(stat = "identity", alpha = 0.8) +
...
salaryplot <- ggplot(testingsalary, aes(salary, pct, colour = Quit, fill = Quit)) +
  geom_bar(stat = "identity", alpha = 0.8) +
...
* Adding more nodes improved the accuracy of our model by 0.5% which is a small improvment. Using the second CART model, lets see the performance of the model by plotting the ROC and precision adn recall curves. 
...
plot(roc.perf, colorize = TRUE, main = "ROC Curve CART Model 2") 
  legend(0.6, 0.2, c(roc.perfauc), border = "white", cex = 1.0, box.col = "white") 
  abline(a = 0, b = 1)
```
...
plot(prec.recall, colorize = TRUE, main = " Precision and Recall Curve CART Model 2")
```
...
plot(roc.perfmodel1, colorize = TRUE, main = "ROC Curve Logistic Regression Model")
...
legend(0.6, 0.2, c(roc.perfmodel1auc), border = "white", cex = 1.0, box.col = "white")
abline(a = 0, b = 1)
...
plot(performance(roc_predmodel1, measure="prec", x.measure="rec"), colorize=TRUE,
     main = "Precision and Recall Curve \nLogistic Regression Model\n ")
...
* Higher satisfaction interacted with number of projects will diminish the chances of quitting. If employees are satisfied, they decrease the chances of quitting due to projects. 
* Higher the evaluation of an employee interacted with high projects count, monthly hours and years hired by company will increase the chances of employees leaving. If employees know their worth, but aren't receiving ideal work conditions they are more likely to quit. 
...
legend(0.6, 0.2, c(roc.perfmodel100auc), border = "white", cex = 1.0, box.col = "white")
abline(a = 0, b = 1)
...
plot(performance(roc_predmodel100, measure="prec", x.measure="rec"), 
     colorize=TRUE)
title("Precision and Recall Curve Logistic Regression \nModel with Interactions")
```
...
legend(0.6, 0.2, c(roc.perfrandom1auc), border = "white", cex = 1.0, box.col = "white")
abline(a = 0, b = 1)
...
```{r, echo=FALSE}
employees <- data.frame(Satisfaction = c(.11,.90),
                                         Evaluation = c(.93,.98),
                                         NumberProjects = c(7,4),
                                         AvgMonthlyHours = c(308,264), 
                                         YearsWithCompany = c(4,6), 
                                         WorkAccident = c("0","0"),
                                         Promotion = c("0","0"),
                                         Department = c("IT","product_mng"),
                                         Salary = c("medium", "medium"))
employees
```
...
```{r, echo=FALSE}
cbind(employees, predict(modelinteraction2, type = "response",
...
```{r, echo=FALSE}
employeeoffer <- data.frame(Satisfaction = c(.11,.90),
                               Evaluation = c(.93,.98),
                               NumberProjects = c(4,4),
                               AvgMonthlyHours = c(275,250), 
                               YearsWithCompany = c(4,6), 
                               WorkAccident = c("0","0"),
                               Promotion = c("0","0"),
                               Department = c("IT","product_mng"),
                               Salary = c("medium", "high"))
cbind(employeeoffer, predict(modelinteraction2, type = "response",
                         se.fit = TRUE, interval = "confidence",
                         newdata = employeeoffer))
```
